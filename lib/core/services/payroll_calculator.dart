...
abstract final class PayrollCalculator {
  ...
  /// 🔑 يرفض **حفظ** راتب بلا مقابله بالدينار — قرار المالك ٣
  ///
  /// «لا يُحفَظ رقم بلا مقابل بالدينار». `net_amount_iqd` هو العمود الذي
  /// تجمعه كل التقارير، فصفرٌ فيه مع صافٍ غير صفري يعني **راتباً يختفي من
  /// كل تقرير** بينما المال خرج من الخزينة فعلاً.
  ///
  /// ⚠️ **ليس فرضاً نظرياً:** مسار «صرف راتب» من بطاقة الموظف كان يكتب
  ///   السطر بلا هذا العمود (وبلا `payment_status`)، فيُنشئ سند صرف حقيقياً
  ///   ويسجّل راتباً قيمتُه صفر ومعلَّماً «مستحقّاً». وهو نفس العطل الذي
  ///   عالجه ترحيل v7 في الصفوف **القديمة** بينما بقي الكاتبُ الحيّ يُنتجه.
  ///   (كُشف 2026-08-26 — راجع ع-٢٨.)
  ///
  /// يُستدعى في **طبقة الـ DAO** لا في الشاشة: أي مسار كتابة جديد يمرّ به.
  static void ensureIqdRecorded({
    required String employeeName,
    required double netSalary,
    required double netSalaryIqd,
  }) {
    // الصافي الصفري مسموح (موظف بلا استحقاق هذا الشهر)، والمقابل الصفري
    // معه سليم. الممنوع أن يوجد صافٍ بلا مقابل.
    if (netSalary != 0 && netSalaryIqd == 0) {
      throw StateError(
        'راتب «$employeeName» يُحفَظ بلا مقابله بالدينار — '
        'كل مبلغ يلزمه ما يعادله بالدينار وإلا اختفى من كل تقرير.',
      );
    }
  }
